        ...
        <el-form-item :label="$t('page.popup.image')" required>
          <!-- <div class="popup__row">
            <el-tooltip placement="top">
              <div slot="content">690x1000<br/>({{ $t('option.hight')}} x {{ $t('option.weight')}})</div>
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        title: [
          { required: true, message: this.$t('page.popup.requiredTitle'), trigger: 'blur' }
        ]
      },
      ui: {
        isSubmiting: false,
        ...
        provinceOptions: [],
        linkItemOptions: targetTypeV2.filter(item => item.value !== 9 && item.value !== 10)
      },
      checkBox: {
