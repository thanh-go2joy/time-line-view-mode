<template>
  <sa-section class="invite-friend" v-loading="ui.isLoading">
...
import { updateInviteFriend, getInviteFriendDetails } from './api'
  ...
  methods: {
    async getInviteFriendDetails () {
      ...
      try {
        ...
        this.form = data.data
        this.form.dailyBudget = data.data.dailyBugget
        this.form.inviteeAmount = data.data.inviteeAmout
        this.form.inviterAmount = data.data.inviterAmout
        this.form.budgetSpent = data.data.buggetSpent
      } catch (error) {
        return false
      } finally {
    ...
    async submit () {
      ...
      try {
        ...
        const { data } = await updateInviteFriend(this.$route.params.sn, formData)
        if (data.code === 1) {
