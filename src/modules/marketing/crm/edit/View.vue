<template>
  <sa-section class="edit-crm">
    <template slot="header" class="edit-crm__filters">
      <div>
        <el-input class="crm-input" v-model="crmContent.title"></el-input>
        <el-input
          class="crm-input"
          v-model="crmContent.memo"
          type="textarea"
        ></el-input>
      </div>
      <div class="float-right">
        ...
        <el-button @click="onExportCRM">
          <i class="el-icon-download" />
          {{ $t('button.export') }}
        </el-button>
        <el-button type="success" @click="onSend">
          <i class="el-icon-s-promotion" />
          {{
            (!$route.query.type ? $t('button.save') + '&' : '') +
            $t('button.send')
          }}
        </el-button>
        <el-button type="success" v-if="!$route.query.type" @click="onSave">
          <i class="el-icon-s-promotion" />
          {{ $t('button.save') }}
        </el-button>
        <router-link
    ...
    <div class="edit-crm__table" v-loading="isLoading">
      <table>
        <tr
...
export default {
  ...
  computed: {
    getBack () {
      return !this.$route.query.page
        ? { name: 'crm' }
        : { name: 'crmEdit', params: { sn: this.$route.params.sn } }
    }
  },
  created () {
    this.getCRM()
  ...
  methods: {
    ...
    async onExportCRM () {
      try {
        this.isLoading = true
        const { data } = await onExportCRM({
          crmFilterSn: this.$route.params.sn
        })
        if (data.code === 1) {
          this.$message({
            type: 'success',
