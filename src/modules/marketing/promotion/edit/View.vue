<template>
  <sa-section v-loading="ui.isLoading">
    <el-form
      ...
      label-position="left"
      label-width="20%"
      class="promotion-edit__content"
      hide-required-asterisk
    >
      <el-form-item :label="$t('page.create-promotion.promotionGroup')">
        <div>
          <el-select
            :loading="ui.isFilterLoading"
            v-model="ui.promotion"
            value-key="sn"
      ...
      <el-form-item :label="$t('page.create-promotion.title')" prop="title">
        <el-input
          type="text"
          name="title"
      ...
      <el-form-item :label="$t('page.create-promotion.code')" required>
        <el-col :span="4" class="pd-l-0">
          <el-form-item prop="code">
            <el-input
              :disabled="$route.query.page !== 'copy'"
              type="text"
              name="code"
      ...
      <el-form-item :label="$t('page.create-promotion.type')">
      ...
      <el-form-item :label="$t('page.create-promotion.applyStartEnd')" required>
        <el-date-picker
      ...
      <el-form-item :label="discountLabel">
        <el-select
      ...
      >
        ...
        <el-currency-input
          ...
          clearable
        />
        <span>{{ promotion.discountType == 1 ? 'VND' : '%' }}</span>
      </el-form-item>
      <el-form-item
      ...
      <el-form-item
        ...
        prop="contentEn"
      >
        <tinymce v-model="promotion.contentEn" name="contentEn" :height="500" />
      </el-form-item>
      <el-form-item
      ...
      <el-form-item :label="$t('page.create-promotion.typeOfDisplay')" required>
        <el-select
      ...
      >
        <el-input
          v-model="promotion.subContentVi"
          type="textarea"
          name="subContentVi"
        />
      </el-form-item>
      ...
      >
        <el-input
          v-model="promotion.subContentEn"
          type="textarea"
          name="subContentEn"
        />
      </el-form-item>
      ...
      >
        <el-input
          v-model="promotion.nameOfButton"
          type="textarea"
          name="nameOfButton"
        />
      </el-form-item>
...
import { onEditPromotion, onCreatePromotion, listGroupPromotion, getPromotion } from './api'
  ...
  computed: {
    ...
    rules () {
      return {
        ...
        content: [
          {
            required: true,
            message: this.$t('page.create-promotion.contentRequired'),
            trigger: 'blur'
          }
        ],
        contentEn: [
        ...
        imageList: [
          {
            required: this.$route.query.page === 'copy',
    ...
    getType () {
      switch (this.promotion.type) {
        case 0:
          return this.$t('option.typePromotion.all')
        case 1:
          return this.$t('option.typePromotion.apply')
        case 2:
          return this.$t('option.typePromotion.event')
        case 3:
          return this.$t('option.typePromotion.inviteFriend')
        case 4:
          return this.$t('option.typePromotion.gift')
        case 5:
          return this.$t('option.typePromotion.booking')
        case 6:
          return this.$t('option.typePromotion.reportNewHotel')
        case 7:
          return this.$t('option.typePromotion.signUp')
        case 8:
          return this.$t('option.typePromotion.birthday')
        case 9:
          return this.$t('option.typePromotion.payment')
        case 10:
          return this.$t('option.typePromotion.voucherCode')
        default:
          return ''
      }
    }
  ...
  methods: {
    async getPromotion () {
      ...
      try {
        ...
        const { couponForUserGroup, imageList, issueCondition, useCondition, voucherCondition, ...dataPtomotion } = data.data
        this.promotion = dataPtomotion
        this.promotion.code = this.$route.query.page === 'copy' ? '' : this.promotion.code
        this.ui.applyStartEnd = this.promotion.numActiveDay > 0 ? 1 : 0 // 1: select start and end coupon date, 0: input num of date available
        this.disableTypeDisplay = !(
          this.promotion.type === 1 ||
          this.promotion.type === 4 ||
          this.promotion.type === 5
        )
        if (!this.$route.query.page) {
          imageList.forEach((value, index) => {
            this.files[index].src = this.formatImage(value.imagePath)
          })
        }
        this.arrPromotionGroups = data.data.listPromotionGroup
      } catch (error) {
        return false
      } finally {
      ...
      this.$refs.ruleForm.validate(async (valid) => {
        if (valid) {
          try {
            ...
            let data
            if (this.$route.query.page !== 'copy') {
              data = await onEditPromotion(this.$route.params.promotionSn, formData)
            } else {
              data = await onCreatePromotion(formData)
            }
            if (data.data.code === 1) {
              this.$router.push(
                {
                  ...
                  params: {
                    couponSn: data.data.data.couponSn,
                    promotionSn: data.data.data.promotionSn
                  }
                })
              this.$message({
                type: 'success',
    ...
    onAddPromotionGroups () {
      this.arrPromotionGroups.push(this.ui.promotion)
      this.ui.promotion = {}
    },
    ...
    filterTypeSelect (data) {
      ...
      // radio check
      if (this.type === 8) {
        this.applyStartEnd = 1
      } else {
        this.applyStartEnd = 0
      }
      // disable typeDisplay
