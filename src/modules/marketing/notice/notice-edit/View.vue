<template>
  <sa-section class="notice" v-loading="ui.isLoading">
...
import { updateNotice, getAppNoticeDetails } from './api'
  ...
  methods: {
    ...
    async submit (isSaveAndSend) {
      ...
      try {
        ...
        const { data } = await updateNotice(this.$route.params.sn, formData)

        if (data.code === 1) {
          if (isSaveAndSend === true) {
            this.$router.push({ name: 'notificationCreate', params: { targetType: 1, targetSn: this.$route.query.sn } })
          } else {
